...
$.extend( $.validator, {
	...
	messages: {
		...
		range: $.validator.format( "请在{0}和{1}之间输入一个值" ),
	...
	classRuleSettings: {
		...
		email: { email: true },
		isMobile: { isMobile: true },  //自定义手机号验证
		isChinese: { isChinese: true },  //自定义中文验证
		url: { url: true },
		date: { date: true },
...
//			return this.optional( element ) || /^[1][0-9]{10}$/.test( value );
		...
		// 自定义账号验证
//		提示语:允许英文字母，数字及_，不能是纯数字
		isAccountNumber: function( value, element ) {
			var isAccount = /^[a-zA-Z0-9_]+$/;
			var isAccountNumbernum = /^[0-9]*$/;
			return this.optional( element ) || isAccount.test( value ) && !isAccountNumbernum.test( value );
		},
		// 自定义密码验证
//		提示语:允许大小写英文字母，数字、"@"、"."、"_"、"-"
